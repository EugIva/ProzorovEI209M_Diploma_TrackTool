...
import AutoFlowFilter
import FlowInfo
#import FlowSorter
import MergerJson
import functions
from DivideJson import divide_json_file
from ExportArtfmJson import convert_json_atfm
from ExportKimCsv import convert_csv_kim
from FlowSorter import FlowSorter
from RoutesDrawerMap import display_tracks, change_data_source, clear_map
...
class TrackTool(TKMT.ThemedTKinterFrame):
    ...
    # настройки
        ...
        self.color_scheme_option = ttk.OptionMenu(self.settings_frame, self.color_scheme_var, "", "dark", "light", command=lambda value: change_color_scheme(self, value))
        ...
        self.clear_exports_button = ttk.Button(self.settings_frame, text="Удалить", command=lambda: functions.clear_exports(self))
    ...
    # ВКЛАДКА "СНИФФЕР"
        ...
        self.callsign_label = ttk.Label(self.filter_frame_lbl, text="По номеру рейса", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.callsign_label.grid(row=0, column=0, padx=10, pady=10)

        self.callsign_switch = ttk.Checkbutton(self.filter_frame_lbl, variable=self.callsign_var, command=lambda: functions.clear_field(self.callsign_entry, self.callsign_var) or (functions.enable_field(self.callsign_entry) if self.callsign_var.get() else functions.disable_field(self.callsign_entry)), style='Switch.TCheckbutton')
        ...
        self.origin_airport_label = ttk.Label(self.filter_frame_lbl, text="По АП вылета", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.origin_airport_label.grid(row=1, column=0, padx=10, pady=10)

        self.origin_airport_switch = ttk.Checkbutton(self.filter_frame_lbl, variable=self.origin_airport_var, command=lambda: functions.clear_field(self.origin_airport_entry, self.origin_airport_var) or (functions.enable_field(self.origin_airport_entry) if self.origin_airport_var.get() else functions.disable_field(self.origin_airport_entry)), style='Switch.TCheckbutton')
        ...
        self.on_ground_label = ttk.Label(self.filter_frame_lbl, text="По положению", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.on_ground_label.grid(row=2, column=0, padx=10, pady=10)

        self.on_ground_switch = ttk.Checkbutton(self.filter_frame_lbl, variable=self.on_ground_var, command=lambda: functions.clear_field(self.on_ground_combobox, self.on_ground_var) or (functions.enable_field(self.on_ground_combobox) if self.on_ground_var.get() else functions.disable_field(self.on_ground_combobox)), style='Switch.TCheckbutton')
        self.on_ground_switch.grid(row=2, column=1, padx=5, pady=10)
        self.on_ground_combobox = ttk.Combobox(self.filter_frame_lbl, values=['В воздухе', 'На земле'], width=37, state="readonly")
        ...
        self.session_time_label = ttk.Label(self.sniffer_frame_lbl, text="Время работы (секунды)", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.session_time_label.grid(row=1, column=0, padx=10, pady=10)
        self.session_time_entry = ttk.Spinbox(self.sniffer_frame_lbl, from_=1.0, to=99999999.0, width=38, textvariable=self.minutes_var)
        self.session_time_entry.grid(row=1, column=1, padx=10, pady=10)

        self.latitude_label = ttk.Label(self.sniffer_frame_lbl, text="Latitude \ Широта", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.latitude_label.grid(row=2, column=0, padx=10, pady=10)
        self.latitude_entry = ttk.Spinbox(self.sniffer_frame_lbl, from_=1.0, to=99999999.0, increment=0.000001, width=38, textvariable=self.latitude_var)
        self.latitude_entry.grid(row=2, column=1, padx=10, pady=10)

        self.longitude_label = ttk.Label(self.sniffer_frame_lbl, text="Longitude \ Долгота", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.longitude_label.grid(row=3, column=0, padx=10, pady=10)
        self.longitude_entry = ttk.Spinbox(self.sniffer_frame_lbl, from_=1.0, to=99999999.0, increment=0.000001, width=38, textvariable=self.longitude_var)
        self.longitude_entry.grid(row=3, column=1, padx=10, pady=10)

        self.radius_label = ttk.Label(self.sniffer_frame_lbl, text="Радиус зоны (метры)", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.radius_label.grid(row=4, column=0, padx=10, pady=10)
        self.radius_entry = ttk.Spinbox(self.sniffer_frame_lbl, from_=1.0, to=99999999.0, increment=1000, width=38, textvariable=self.radius_var)
        self.radius_entry.grid(row=4, column=1, padx=10, pady=10)

        self.filename_label = ttk.Label(self.sniffer_frame_lbl, text="Имя файла", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.folder_label = ttk.Label(self.sniffer_frame_lbl, text="Файл будет создан в: Exports/sniffer/", font=self.font_txt_light, foreground='grey', justify="left", anchor="w")
        self.folder_label.grid(row=6, column=1, columnspan=1, padx=5, pady=0)

        self.start_button = ttk.Button(self.button_frame, text="Запустить", compound='left', command=lambda: functions.launch_timer(self.session_time_entry, self.latitude_entry, self.longitude_entry, self.radius_entry, self.filename_entry, self.callsign_entry, self.origin_airport_entry, self.on_ground_combobox, self.file_info_label), style='Accent.TButton')
        self.start_button.grid(row=8, column=0, padx=10, pady=30)

        self.open_folder_button = ttk.Button(self.button_frame, text=" Открыть папку ", compound='left', command=lambda: functions.open_folder("sniffer"))
        ...
        self.sniffer_info_button = ttk.Button(self.button_frame, text="⍰", command=lambda: functions.show_info(self, self.help_sniffer), style='Toolbutton')
        ...
        self.file_info_label = ttk.Label(self.sniffer_result_frame, text="", font=self.font_txt, foreground='grey', justify="left", anchor="w")
    ...
    # Вкладка "Слепок"
        ...
        self.city_label_trail = ttk.Label(self.trail_frame_upper_lbl, text="Город (имя точки)", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.radius_label_trail = ttk.Label(self.trail_frame_upper_lbl, text="Радиус зоны (метры)", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.radius_label_trail.grid(row=1, column=0, padx=10, pady=10)
        self.radius_entry_trail = ttk.Spinbox(self.trail_frame_upper_lbl, from_=1.0, to=99999999.0, increment=1000, width=38, textvariable=self.radius_var_trail)
        self.radius_entry_trail.grid(row=1, column=1, padx=10, pady=10)

        self.latitude_label_trail = ttk.Label(self.trail_frame_upper_lbl, text="Latitude \ Широта", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.latitude_label_trail.grid(row=2, column=0, padx=10, pady=10)
        self.latitude_entry_trail = ttk.Spinbox(self.trail_frame_upper_lbl, from_=1.0, to=99999999.0, increment=0.000001, width=38, textvariable=self.latitude_var_trail)
        self.latitude_entry_trail.grid(row=2, column=1, padx=10, pady=10)

        self.longitude_label_trail = ttk.Label(self.trail_frame_upper_lbl, text="Longitude \ Долгота", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.longitude_label_trail.grid(row=3, column=0, padx=10, pady=10)
        self.longitude_entry_trail = ttk.Spinbox(self.trail_frame_upper_lbl, from_=1.0, to=99999999.0, increment=0.000001, width=38, textvariable=self.longitude_var_trail)
        ...
        self.trail_add_coords_button = ttk.Button(self.trail_table_buttons_lbl, text=" Добавить ", command=lambda: functions.add_coords_to_table(self.trails_treeview, self.city_entry_trail, self.radius_entry_trail, self.latitude_entry_trail, self.longitude_entry_trail))
        self.trail_add_coords_button.grid(row=0, column=0, padx=20, pady=5, sticky="ew")

        self.trail_delete_coords_button = ttk.Button(self.trail_table_buttons_lbl, text=" Удалить ", command=lambda: functions.delete_coords_from_table(self.trails_treeview))
        self.trail_delete_coords_button.grid(row=1, column=0, padx=20, pady=5, sticky="ew")

        self.trail_import_coords_button = ttk.Button(self.trail_table_buttons_lbl, text=" Импорт ", command=lambda: functions.import_coords_from_file(self.trails_treeview))
        self.trail_import_coords_button.grid(row=2, column=0, padx=20, pady=5, sticky="ew")

        self.trail_export_coords_button = ttk.Button(self.trail_table_buttons_lbl, text=" Экспорт ", command=lambda: functions.export_coords_to_file(self.trails_treeview))
        ...
        self.trails_yscrollbar = ttk.Scrollbar(self.trail_table_lbl, orient="vertical", command=self.trails_treeview.yview)
        ...
        self.trails_treeview.heading("#0", text="№", command=lambda: functions.sort_by_column(self, self.treeview, "#0", 0))
        self.trails_treeview.heading("City", text="City", command=lambda: functions.sort_by_column(self, self.trails_treeview, "City", 0))
        self.trails_treeview.heading("Radius", text="Radius", command=lambda: functions.sort_by_column(self, self.trails_treeview, "Radius", 0))
        self.trails_treeview.heading("Latitude", text="Latitude", command=lambda: functions.sort_by_column(self, self.trails_treeview, "Latitude", 0))
        self.trails_treeview.heading("Longitude", text="Longitude", command=lambda: functions.sort_by_column(self, self.trails_treeview, "Longitude", 0))


        self.pause_trail_label = ttk.Label(self.trail_frame_upper_lbl, text="Пауза (сек)", font=self.font_txt, foreground='grey')
        self.pause_trail_label.grid(row=5, column=0, padx=10, pady=10)

        self.pause_trail_entry = ttk.Spinbox(self.trail_frame_upper_lbl, width=38, from_=0.0, to=99999999.0, increment=60, textvariable=self.pause_var_trail_coords)
        self.pause_trail_entry.grid(row=5, column=1, padx=10, pady=10)


        self.filename_label_trail = ttk.Label(self.trail_frame_lower_lbl, text="  Имя файла сеанса ", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.folder_label_trail = ttk.Label(self.trail_frame_lower_lbl, text="Файл будет создан в: Exports/trails/", font=self.font_txt_light, foreground='grey', justify="left", anchor="w")
        ...
        self.iterations_label = ttk.Label(self.duration_frame_lbl, text="Количество итераций", font=self.font_txt, foreground='grey')
        self.iterations_label.grid(row=1, column=0, padx=10, pady=10)

        self.iterations_entry = ttk.Spinbox(self.duration_frame_lbl, width=20, state='disabled', from_=2.0, to=99999999.0, increment=1)
        ...
        self.pause_entry = ttk.Spinbox(self.duration_frame_lbl, width=20, state='disabled', from_=0.0, to=99999999.0, increment=60)
        ...
        self.start_button = ttk.Button(self.button_frame_lbl, text="Снять данные",
                    command=lambda: functions.launch_trail_getter(self.trails_treeview, self.filename_entry_trail, self.iterations_entry, self.pause_entry, self.pause_trail_entry, self.file_info_label_trail),
                    style='Accent.TButton')
        self.start_button.grid(row=0, column=0, padx=130, pady=10, columnspan=2, sticky="ew")

        self.open_folder_button = ttk.Button(self.button_frame_lbl, text=" Открыть папку ", command=lambda: functions.open_folder("trails"))
        self.open_folder_button.grid(row=3, column=0, padx=130, pady=20, sticky="ew")

        self.stamp_info_button = ttk.Button(self.button_frame_lbl, text="⍰", command=lambda: functions.show_info(self, self.help_trails), style='Toolbutton')
        self.stamp_info_button.grid(row=4, column=0, padx=130, pady=10, sticky="ew")

        self.duration_time_label = ttk.Label(self.button_frame_lbl, text="Времени займёт ≈", font=self.font_txt, foreground='grey')
        self.duration_time_label.grid(row=1, column=0, padx=130, pady=0, sticky="ew")

        self.duration_time_value_label = ttk.Label(self.button_frame_lbl, text="", font=self.font_txt, foreground='grey')
    ...
    # ВКЛАДКА excel
        self.file_label = ttk.Label(self.excel_tab, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.file_label.grid(row=0, column=0, padx=10, pady=10)

        self.file_entry = ttk.Entry(self.excel_tab, width=60)
        self.file_entry.grid(row=0, column=1, padx=10, pady=10)

        self.file_button = ttk.Button(self.excel_tab, text="Поиск", command=lambda: functions.find_file(self.file_entry, "Выбор файла для конвертации"))
        self.file_button.grid(row=0, column=2, padx=10, pady=10)

        self.convert_button = ttk.Button(self.excel_tab, text=" Преобразовать ",  compound='left', command=lambda: functions.start_programm('DataToExcel', 'convert_json_excel', self.file_entry.get()), style='Accent.TButton')
        self.convert_button.grid(row=1, column=1, padx=10, pady=10)

        self.open_folder_button_excel = ttk.Button(self.excel_tab, text="Открыть папку",  compound='left', command=lambda: functions.open_folder("excelExports"))
        self.open_folder_button_excel.grid(row=2, column=1, padx=10, pady=10)

        self.excel_info_button = ttk.Button(self.excel_tab, text="⍰", command=lambda: functions.show_info(self, self.help_excel), style='Toolbutton')
        self.excel_info_button.grid(row=0, column=3, padx=5, pady=5)

    # Вкладка "Разделение JSON"
        self.file_label_divide = ttk.Label(self.divide_json_tab, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_button_divide = ttk.Button(self.divide_json_tab, text="Поиск", command=lambda: functions.find_file(self.file_entry_divide, "Выбор JSON файла для порейсового извлечения новых JSON"))
        self.file_button_divide.grid(row=0, column=2, padx=10, pady=10)

        self.divide_button = ttk.Button(self.divide_json_tab, text=" Разделить JSON ", compound='left', command=lambda: divide_json_file(self.file_entry_divide.get()), style='Accent.TButton')
        self.divide_button.grid(row=1, column=1, padx=10, pady=10)

        self.open_folder_button_divide = ttk.Button(self.divide_json_tab, text="Открыть папку", compound='left', command=lambda: functions.open_folder("jsonDivide"))
        self.open_folder_button_divide.grid(row=2, column=1, padx=10, pady=10)

        self.divide_info_button = ttk.Button(self.divide_json_tab, text="⍰", command=lambda: functions.show_info(self, self.help_jsonDivide), style='Toolbutton')
        self.divide_info_button.grid(row=0, column=3, padx=5, pady=5)

    # ВКЛАДКА "объединение Json"
        self.file_label_merger = ttk.Label(self.json_merger_tab, text="Выберите файл(ы):", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_button_merger = ttk.Button(self.json_merger_tab, text="Поиск", command=lambda: MergerJson.find_json_files(self))
        self.file_button_merger.grid(row=0, column=2, padx=10, pady=10)

        self.merger_button = ttk.Button(self.json_merger_tab, text="Создать JSON", compound='left', command=lambda: MergerJson.merge_json_files(self), style='Accent.TButton')
        self.merger_button.grid(row=0, column=3, padx=10, pady=10)

        self.open_folder_button_merger = ttk.Button(self.json_merger_tab, text="Открыть папку", compound='left', command=lambda: functions.open_folder("jsonMerge"))
        self.open_folder_button_merger.grid(row=0, column=4, padx=10, pady=10)

        self.merger_info_button = ttk.Button(self.json_merger_tab, text="⍰", command=lambda: functions.show_info(self, self.help_jsonMerge), style='Toolbutton')
        self.merger_info_button.grid(row=0, column=5, padx=5, pady=5)

        self.file_label_merger = ttk.Label(self.json_merger_tab, text="Введите название:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.treeview = ttk.Treeview(self.listbox_frame, columns=("id", "callsign", "flight_time", "route_points", "airport_origin", "airport_destination"))
        ...
        self.delete_button = ttk.Button(self.listbox_frame, text="Удалить", command=lambda: MergerJson.delete_selected_items(self))
        ...
        self.treeview.heading("#0", text="File Path", command=lambda: functions.sort_by_column(self, self.treeview, "#0", 0))
        self.treeview.heading("id", text="ID", command=lambda: functions.sort_by_column(self, self.treeview, "id", 0))
        self.treeview.heading("callsign", text="Callsign", command=lambda: functions.sort_by_column(self, self.treeview, "callsign", 0))
        self.treeview.heading("flight_time", text="Flight Time", command=lambda: functions.sort_by_column(self, self.treeview, "flight_time", 0))
        self.treeview.heading("route_points", text="Route Points", command=lambda: functions.sort_by_column(self, self.treeview, "route_points", 0))
        self.treeview.heading("airport_origin", text="Airport Origin", command=lambda: functions.sort_by_column(self, self.treeview, "airport_origin", 0))
        self.treeview.heading("airport_destination", text="Airport Destination", command=lambda: functions.sort_by_column(self, self.treeview, "airport_destination", 0))
        ...
        self.file_info_label_merger = ttk.Label(self.merger_result_frame, text="", font=self.font_txt, foreground='grey')
        self.file_info_label_merger.grid(row=0, column=0, columnspan=3, padx=10, pady=0, sticky="nsew")

    # ВКЛАДКА drawer2D
        self.file_label2D = ttk.Label(self.drawer2D_tab, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_button2D = ttk.Button(self.drawer2D_tab, text="Поиск", command=lambda: functions.find_file(self.file_entry2D, "Выбор файла для отрисовки треков на плоскости"))
        self.file_button2D.grid(row=0, column=2, padx=10, pady=10)

        self.drawer2d_button = ttk.Button(self.drawer2D_tab, text=" Отрисовать треки в 2D ",  compound='left', command=lambda: functions.start_programm('RoutesDrawer', 'RoutesDrawer2D', self.file_entry2D.get()), style='Accent.TButton')
        self.drawer2d_button.grid(row=1, column=1, padx=10, pady=10)

        self.open_folder_button2D = ttk.Button(self.drawer2D_tab, text="Открыть папку",  compound='left', command=lambda: functions.open_folder("drawRoutes2D"))
        self.open_folder_button2D.grid(row=2, column=1, padx=10, pady=10)

        self.drawer2D_info_button = ttk.Button(self.drawer2D_tab, text="⍰", command=lambda: functions.show_info(self, self.help_draw2D), style='Toolbutton')
        self.drawer2D_info_button.grid(row=0, column=3, padx=5, pady=5)

    #ВКЛАДКА drawerVertical
        self.file_labelVertical = ttk.Label(self.drawerVertical_tab, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_buttonVertical = ttk.Button(self.drawerVertical_tab, text="Поиск", command=lambda: functions.find_file(self.file_entryVertical, "Выбор файла для отрисовки вертикальных профилей треков"))
        self.file_buttonVertical.grid(row=0, column=2, padx=10, pady=10)

        self.drawerVertical_button = ttk.Button(self.drawerVertical_tab, text=" Отрисовать вертикальные профили", compound='left', command=lambda: functions.start_programm('VerticalProfileDrawer', 'vertical_routes_drawer', self.file_entryVertical.get()), style='Accent.TButton')
        self.drawerVertical_button.grid(row=1, column=1, padx=10, pady=10)

        self.open_folder_buttonVertical = ttk.Button(self.drawerVertical_tab, text="Открыть папку",  compound='left', command=lambda: functions.open_folder("drawVerticalProfiles"))
        self.open_folder_buttonVertical.grid(row=2, column=1, padx=10, pady=10)

        self.drawerVertical_info_button = ttk.Button(self.drawerVertical_tab, text="⍰", command=lambda: functions.show_info(self, self.help_drawVert), style='Toolbutton')
        self.drawerVertical_info_button.grid(row=0, column=3, padx=5, pady=5)

    #ВКЛАДКА drawer3D
        self.file_label3D = ttk.Label(self.drawer3D_tab, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_button3D = ttk.Button(self.drawer3D_tab, text="Поиск", command=lambda: functions.find_file(self.file_entry3D, "Выбор файла для отрисовки треков в пространственном графике"))
        self.file_button3D.grid(row=0, column=2, padx=10, pady=10)

        self.drawer3D_button = ttk.Button(self.drawer3D_tab, text=" Отрисовать треки в 3D ",  compound='left', command=lambda: functions.start_programm('RoutesDrawer3D', 'routes_drawer_3D', self.file_entry3D.get()), style='Accent.TButton')
        self.drawer3D_button.grid(row=1, column=1, padx=10, pady=10)

        self.open_folder_button3D = ttk.Button(self.drawer3D_tab, text="Открыть папку", compound='left', command=lambda: functions.open_folder("drawRoutes3D"))
        self.open_folder_button3D.grid(row=2, column=1, padx=10, pady=10)

        self.drawer3D_info_button = ttk.Button(self.drawer3D_tab, text="⍰", command=lambda: functions.show_info(self, self.help_draw3D), style='Toolbutton')
    ...
    #ВКЛАДКА MAP
        ...
        self.file_path_label = ttk.Label(frame, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_buttonMap = ttk.Button(frame, text="Поиск", command=lambda: functions.find_file(self.file_path_entry, "Выбор файла для отрисовки треков на карте"))
        self.file_buttonMap.pack(side="left", padx=5)

        self.display_button = ttk.Button(frame, text="Отобразить", command=lambda: display_tracks(self, self.file_path_entry.get()), style='Accent.TButton')
        self.display_button.pack(side="left", padx=5)

        self.clear_button = ttk.Button(frame, text="Очистить", command=lambda: clear_map(self.map_widget.delete_all_path(), self.map_widget.delete_all_marker()))
        self.clear_button.pack(side="left", padx=5)

        self.data_source_label = ttk.Label(frame, text="Изображение карты:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.data_source_label.pack(side="left", padx=15)

        self.data_source_var = tk.StringVar()
        self.data_source_var.set("OpenStreetMap")

        self.data_source_option = ttk.OptionMenu(frame, self.data_source_var, "OpenStreetMap", "OpenStreetMap", "Google Maps", "Google спутник", "Google рельеф", command=lambda value: change_data_source(self, value))
        ...
        self.map_info_button = ttk.Button(frame, text="⍰", command=lambda: functions.show_info(self, self.help_drawMap), style='Toolbutton')
    ...
    #график скорости
        ...
        self.file_label_speed_graph = ttk.Label(self.speed_graph_frame, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_button_speed_graph = ttk.Button(self.speed_graph_frame, text="Поиск", command=lambda: functions.find_file(self.file_entry_speed_graph, "Выбор JSON файла для рисовки графика скорости"))
        self.file_button_speed_graph.grid(row=0, column=2, padx=10, pady=10)

        self.draw_button_speed_graph = ttk.Button(self.speed_graph_frame, text=" Отрисовать график скорости ", command=lambda: functions.start_programm('SpeedDrawer', 'draw_speed_graph', self.file_entry_speed_graph.get(), self.speed_type_var), style='Accent.TButton')
        self.draw_button_speed_graph.grid(row=3, column=1, padx=10, pady=10)

        self.open_folder_button_speed_graph = ttk.Button(self.speed_graph_frame, text="Открыть папку", command=lambda: functions.open_folder("drawSpeed"))
        self.open_folder_button_speed_graph.grid(row=4, column=1, padx=10, pady=10)

        self.speed_graph_info_button = ttk.Button(self.speed_graph_frame, text="⍰", command=lambda: functions.show_info(self, self.help_drawSpeed), style='Toolbutton')
        ...
        self.speed_type_label = ttk.Label(self.speed_graph_frame, text="Тип скорости:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.speed_type_label.grid(row=1, column=0, padx=10, pady=10)

        self.speed_type_option = ttk.OptionMenu(self.speed_graph_frame, self.speed_type_var, "groundSpeed_Kts", "groundSpeed_Kts", "verticalSpeed")
    ...
    # экспорт для ATFM
        ...
        self.file_label_atfm = ttk.Label(self.export_to_atfm_frame, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_button_atfm = ttk.Button(self.export_to_atfm_frame, text="Поиск", command=lambda: functions.find_file(self.file_entry_atfm, "Выбор JSON файла для преобразования и импорта в ATFM"))
        self.file_button_atfm.grid(row=0, column=2, padx=10, pady=10)

        self.convert_button_atfm = ttk.Button(self.export_to_atfm_frame, text=" Экспорт в JSON для ATFM-model ", style='Accent.TButton',
                                              command=lambda: convert_json_atfm(self.file_entry_atfm.get(),
        ...
        self.open_folder_button_atfm = ttk.Button(self.export_to_atfm_frame, text="Открыть папку", command=lambda: functions.open_folder("jsonATFM"))
        self.open_folder_button_atfm.grid(row=0, column=4, padx=10, pady=10)

        self.atfm_info_button = ttk.Button(self.export_to_atfm_frame, text="⍰", command=lambda: functions.show_info(self, self.help_atfm), style='Toolbutton')
        ...
        self.name_label = ttk.Label(self.settings_label_frame, text="Название ", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.name_label.grid(row=0, column=0, padx=10, pady=10)

        self.name_entry = ttk.Entry(self.settings_label_frame, width=55, textvariable=self.atfm_name_var)
        self.name_entry.grid(row=0, column=1, padx=10, pady=10)

        self.commentary_label = ttk.Label(self.settings_label_frame, text="Комментарий ", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.catalog_id_label = ttk.Label(self.settings_label_frame, text="ID каталога ", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.user_id_label = ttk.Label(self.settings_label_frame, text="ID пользователя ", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.number_label = ttk.Label(self.settings_label_frame, text="Номер ", font=self.font_txt, foreground='grey', justify="right", anchor="e")
    ...
    # вкладка Экспорт в csv для КИМ
        ...
        self.file_label_kim = ttk.Label(self.export_to_kim_frame, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_button_kim = ttk.Button(self.export_to_kim_frame, text="Поиск", command=lambda: [functions.find_file(self.file_entry_kim, "Выбор JSON файла для преобразования и импорта в ATFM"), update_filename_on_button_click(self)])
        self.file_button_kim.grid(row=0, column=2, padx=10, pady=10)

        self.convert_button_kim = ttk.Button(self.export_to_kim_frame, text=" Экспорт в CSV для КИМ ", style='Accent.TButton',
        ...
        self.open_folder_button_kim = ttk.Button(self.export_to_kim_frame, text="Открыть папку", command=lambda: functions.open_folder("csvKIM"))
        self.open_folder_button_kim.grid(row=0, column=4, padx=10, pady=10)

        self.kim_info_button = ttk.Button(self.export_to_kim_frame, text="⍰", command=lambda: functions.show_info(self, self.help_kim), style='Toolbutton')
        ...
        self.filename_label_kim = ttk.Label(self.settings_label_frame_kim, text="Название файла", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.filename_label_kim.grid(row=0, column=0, padx=10, pady=10)

        self.filename_entry_kim = ttk.Entry(self.settings_label_frame_kim, width=55)#os.path.splitext(self.file_entry_kim)[0])
        self.filename_entry_kim.grid(row=0, column=1, padx=10, pady=10)
        self.filename_entry_kim.config(state="readonly")

        self.name_label_kim = ttk.Label(self.settings_label_frame_kim, text="Название потока", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.commentary_label_kim = ttk.Label(self.settings_label_frame_kim, text="Комментарий ", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.commentary_label_kim.grid(row=2, column=0, padx=10, pady=10)

        self.commentary_entry_kim = ttk.Entry(self.settings_label_frame_kim, width=55, textvariable=self.atfm_comment_var)
        self.commentary_entry_kim.grid(row=2, column=1, padx=10, pady=10)

        self.catalog_id_label_kim = ttk.Label(self.settings_label_frame_kim, text="ID каталога ", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.catalog_id_label_kim.grid(row=3, column=0, padx=10, pady=10)

        self.catalog_id_entry_kim = ttk.Entry(self.settings_label_frame_kim, width=55, textvariable=self.atfm_catalog_var)
        self.catalog_id_entry_kim.grid(row=3, column=1, padx=10, pady=10)

        self.variant_id_label_kim = ttk.Label(self.settings_label_frame_kim, text="№ варианта ", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.russia_code_label_kim = ttk.Label(self.settings_label_frame_kim, text="Russia code: 'RR'", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.russia_code_label_kim.grid(row=5, column=0, padx=10, pady=10)

        self.russia_code_var = tk.IntVar()
        self.russia_code_var_checkbox = ttk.Checkbutton(self.settings_label_frame_kim, variable=self.russia_code_var, style='Switch.TCheckbutton')
    ...
    # Вкладка "Информация о потоке"
        ...
        self.file_label_flow_info = ttk.Label(self.flow_info_frame_btns, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_button_flow_info = ttk.Button(self.flow_info_frame_btns, text="Поиск", command=lambda: functions.find_file(self.file_entry_flow_info, "Выбор JSON файла для отображения информации"))
        ...
        self.display_button_flow_info = ttk.Button(self.flow_info_frame_btns, text="Отобразить информацию",
                                                  command=lambda: FlowInfo.display_flow_info(self.file_entry_flow_info.get(),
                                                                                    self.flow_info_label,
                                                                                    None),
                                                  style='Accent.TButton')
        self.display_button_flow_info.grid(row=1, column=1, padx=10, pady=10)

        self.flow_info_info_button = ttk.Button(self.flow_info_frame_btns, text="⍰", command=lambda: functions.show_info(self, self.help_flowInfo), style='Toolbutton')
        ...
        self.flow_info_label = ttk.Label(self.flow_info_frame_result, text="", font=self.font_txt, foreground='grey', justify="left", anchor="w")
...
# Вкладка "Авто-фильтрация потока"
        ...
        self.file_label_auto_filter = ttk.Label(self.auto_filter_frame, text="Выберите файл:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.file_label_auto_filter.grid(row=0, column=0, padx=10, pady=10)

        self.file_button_auto_filter = ttk.Button(self.auto_filter_frame, text="Поиск", command=lambda: functions.find_file(self.file_entry_auto_filter, "Выбор JSON файла для авто-фильтрации потока"))
        self.file_button_auto_filter.grid(row=0, column=2, padx=10, pady=10)

        self.filter_button = ttk.Button(self.auto_filter_frame, text="Фильтровать поток", command=lambda: AutoFlowFilter.filter_flow(self, self.file_entry_auto_filter.get(), self.filtered_info_label), style='Accent.TButton')
        self.filter_button.grid(row=0, column=3, padx=10, pady=10)

        self.open_folder_button_filter = ttk.Button(self.auto_filter_frame, text="Открыть папку", command=lambda: functions.open_folder("flowFiltered"))
        self.open_folder_button_filter.grid(row=0, column=4, padx=10, pady=10)

        self.auto_filter_info_button = ttk.Button(self.auto_filter_frame, text="⍰", command=lambda: functions.show_info(self, "help_flowFilter"), style='Toolbutton')
        ...
        self.filtered_info_label = ttk.Label(self.results_filter_frame, text="", font=self.font_txt, foreground='grey', justify="left", anchor="w")
        self.filtered_info_label.grid(row=0, column=0, columnspan=5, padx=10, pady=0)

        self.min_points_var = tk.IntVar()
        self.min_points_checkbox = ttk.Checkbutton(self.filter_frame, text="Точек маршрута не менее:", variable=self.min_points_var, command=lambda: AutoFlowFilter.update_min_points_entry(self, self.min_points_var, self.min_points_entry), style='Switch.TCheckbutton')
        ...
        self.min_distance_checkbox = ttk.Checkbutton(self.filter_frame, text="Километров м-ду точками маршрута не более:", variable=self.min_distance_var, command=lambda: AutoFlowFilter.update_min_distance_entry(self, self.min_distance_var, self.min_distance_entry), style='Switch.TCheckbutton')
        ...
        self.flight_time_checkbox = ttk.Checkbutton(self.filter_frame, text="Время полёта (мин) не менее:", variable=self.flight_time_var, command=lambda: AutoFlowFilter.update_flight_time_entry(self, self.flight_time_var, self.flight_time_entry), style='Switch.TCheckbutton')
        ...
        self.airport_origin_checkbox_named = ttk.Checkbutton(self.filter_frame, text="Аэропорт вылета:", variable=self.airport_origin_checkbox_var, command=lambda: AutoFlowFilter.update_airport_origin_checkbox(self, self.airport_origin_checkbox_var, self.airport_origin_entry, self.airport_origin_checkbox), style='Switch.TCheckbutton')
        ...
        self.airport_destination_checkbox_named = ttk.Checkbutton(self.filter_frame, text="Аэропорт прилёта:", variable=self.airport_destination_checkbox_var, command=lambda: AutoFlowFilter.update_airport_destination_checkbox(self, self.airport_destination_checkbox_var, self.airport_destination_entry, self.airport_destination_checkbox), style='Switch.TCheckbutton')
        ...
        self.takeoff_to_landing_checkbox_named = ttk.Checkbutton(self.filter_frame, text="Порог высоты в начале и конце трека менее (м):", variable=self.takeoff_to_landing_var, command=lambda: AutoFlowFilter.update_takeoff_to_landing_entry(self, self.takeoff_to_landing_var, self.takeoff_to_landing_entry), style='Switch.TCheckbutton')
        ...
        self.bort_number_checkbox = ttk.Checkbutton(self.filter_frame, text="Указан номер борта", variable=self.bort_number_var, style='Switch.TCheckbutton')
        self.bort_number_checkbox.grid(row=0, column=3, padx=10, pady=10, sticky="w")

        self.no_missing_coords_var = tk.IntVar()
        self.no_missing_coords_checkbox = ttk.Checkbutton(self.filter_frame, text="Нет пропущенных координат", variable=self.no_missing_coords_var, style='Switch.TCheckbutton')
        self.no_missing_coords_checkbox.grid(row=1, column=3, padx=10, pady=10, sticky="w")

        self.no_missing_altitude_var = tk.IntVar()
        self.no_missing_altitude_checkbox = ttk.Checkbutton(self.filter_frame, text="Нет пропущенной высоты", variable=self.no_missing_altitude_var, style='Switch.TCheckbutton')
        self.no_missing_altitude_checkbox.grid(row=2, column=3, padx=10, pady=10, sticky="w")

        self.airport_origin_var = tk.IntVar()
        self.airport_origin_checkbox = ttk.Checkbutton(self.filter_frame, text="Указан аэропорт вылета", variable=self.airport_origin_var, command=lambda: AutoFlowFilter.update_airport_origin(self, self.airport_origin_var, self.airport_origin_checkbox_named, self.airport_origin_entry), style='Switch.TCheckbutton')
        self.airport_origin_checkbox.grid(row=3, column=3, padx=10, pady=10, sticky="w")

        self.airport_destination_var = tk.IntVar()
        self.airport_destination_checkbox = ttk.Checkbutton(self.filter_frame, text="Указан аэропорт прилёта", variable=self.airport_destination_var, command=lambda: AutoFlowFilter.update_airport_destination(self, self.airport_destination_var, self.airport_destination_checkbox_named, self.airport_destination_entry), style='Switch.TCheckbutton')
        self.airport_destination_checkbox.grid(row=4, column=3, padx=10, pady=10, sticky="w")

        self.speed_var = tk.IntVar()
        self.speed_checkbox = ttk.Checkbutton(self.filter_frame, text="Указана скорость", variable=self.speed_var, style='Switch.TCheckbutton')
        self.speed_checkbox.grid(row=0, column=4, padx=10, pady=10, sticky="w")

        self.code_reys_var = tk.IntVar()
        self.code_reys_checkbox = ttk.Checkbutton(self.filter_frame, text="Указан код рейса", variable=self.code_reys_var, style='Switch.TCheckbutton')
        self.code_reys_checkbox.grid(row=1, column=4, padx=10, pady=10, sticky="w")

        self.type_vs_var = tk.IntVar()
        self.type_vs_checkbox = ttk.Checkbutton(self.filter_frame, text="Указан тип ВС", variable=self.type_vs_var, style='Switch.TCheckbutton')
        self.type_vs_checkbox.grid(row=2, column=4, padx=10, pady=10, sticky="w")

        self.cut_altitude_var = tk.IntVar()
        self.cut_altitude_checkbox = ttk.Checkbutton(self.filter_frame, text="Обрезать высоту 0м", variable=self.cut_altitude_var, style='Switch.TCheckbutton')
        self.cut_altitude_checkbox.grid(row=3, column=4, padx=10, pady=10, sticky="w")
...
# Вкладка "ручная сортировка потока"
        ...
        self.file_label_sorting = ttk.Label(self.manual_sorting_frame, text="Выберите файл(ы):", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        ...
        self.file_button_sorting = ttk.Button(self.manual_sorting_frame, text="Поиск", command=lambda: flow_sorter.find_json_files())
        self.file_button_sorting.grid(row=0, column=2, padx=10, pady=10)

        self.sorting_button = ttk.Button(self.manual_sorting_frame, text="Отсортировать", compound='left', command=lambda: flow_sorter.export_sorted_files(), style='Accent.TButton')
        self.sorting_button.grid(row=0, column=3, padx=10, pady=10)

        self.open_folder_button_sorting = ttk.Button(self.manual_sorting_frame, text="Открыть папку", compound='left', command=lambda: functions.open_folder("flowSorted"))
        self.open_folder_button_sorting.grid(row=0, column=4, padx=10, pady=10)

        self.sorting_info_button = ttk.Button(self.manual_sorting_frame, text="⍰", command=lambda: functions.show_info(self, self.help_flowSorter), style='Toolbutton')
        self.sorting_info_button.grid(row=0, column=5, padx=5, pady=5)

        self.file_label_sorting = ttk.Label(self.manual_sorting_frame, text="Введите название:", font=self.font_txt, foreground='grey', justify="right", anchor="e")
        self.file_label_sorting.grid(row=1, column=0, padx=10, pady=10)

        self.output_file_entry_sorting = ttk.Entry(self.manual_sorting_frame, width=60, textvariable=self.sorted_name_var)
        ...
        self.treeview_sorting = ttk.Treeview(self.listbox_frame_sorting, columns=("id", "callsign", "flight_time", "route_points", "airport_origin", "airport_destination"))
        self.treeview_sorting.grid(row=0, column=0, sticky="nsew", padx=20, pady=20)

        self.xscrollbar_sorting = ttk.Scrollbar(self.listbox_frame_sorting, orient="horizontal", command=self.treeview_sorting.xview)
        self.xscrollbar_sorting.grid(row=1, column=0, sticky="ew")

        self.yscrollbar_sorting = ttk.Scrollbar(self.listbox_frame_sorting, orient="vertical", command=self.treeview_sorting.yview)
        self.yscrollbar_sorting.grid(row=0, column=1, sticky="ns")

        self.treeview_sorting.config(xscrollcommand=self.xscrollbar_sorting.set, yscrollcommand=self.yscrollbar_sorting.set)
        ...
        self.treeview_sorting.heading("#0", text="File Path", command=lambda: functions.sort_by_column(self, self.treeview_sorting, "#0", 0))
        self.treeview_sorting.heading("id", text="ID", command=lambda: functions.sort_by_column(self, self.treeview_sorting, "id", 0))
        self.treeview_sorting.heading("callsign", text="Callsign", command=lambda: functions.sort_by_column(self, self.treeview_sorting, "callsign", 0))
        self.treeview_sorting.heading("flight_time", text="Flight Time", command=lambda: functions.sort_by_column(self, self.treeview_sorting, "flight_time", 0))
        self.treeview_sorting.heading("route_points", text="Route Points", command=lambda: functions.sort_by_column(self, self.treeview_sorting, "route_points", 0))
        self.treeview_sorting.heading("airport_origin", text="Airport Origin", command=lambda: functions.sort_by_column(self, self.treeview_sorting, "airport_origin", 0))
        self.treeview_sorting.heading("airport_destination", text="Airport Destination", command=lambda: functions.sort_by_column(self, self.treeview_sorting, "airport_destination", 0))
        ...
        style_correct.configure("Correct.TButton", font=("Cooper Black", 13), foreground="DeepSkyBlue", background="green")

        style_incorrect = ttk.Style()
        style_incorrect.configure("Incorrect.TButton", font=("Cooper Black", 13), foreground="MediumPurple", background="green")

        self.correct_pack_button = ttk.Button(self.button_frame_sorting, text="Корректный", command=lambda: flow_sorter.move_to_correct(), style="Correct.TButton")
        self.correct_pack_button.grid(row=0, column=2, padx=20, pady=20, sticky="ew", ipadx=6, ipady=6)

        self.incorrect_pack_button = ttk.Button(self.button_frame_sorting, text="Испорченный", command=lambda: flow_sorter.move_to_incorrect(), style="Incorrect.TButton")
        ...
        self.undo_button = ttk.Button(self.remover_frame_sorting, text="Отменить шаг", command=lambda: flow_sorter.undo_last_move())
        self.undo_button.grid(row=0, column=1, padx=2, pady=20, sticky="ew", ipadx=0, ipady=0)

        self.undo_button = ttk.Button(self.remover_frame_sorting, text="Очистить всё", command=lambda: flow_sorter.clear_sort())
